...
pub fn colormap_data(colormap_name: &ColorMapSchemes) -> [[f32; 3]; 11] {
    ...
    let colors = match colormap_name {
        isv => [[1.0,0.0,0.0], [1.0,0.5,0.0], [0.97,1.0,0.01], [0.0,0.99,0.04], [0.0,0.98,0.52], [0.0,0.98,1.0], [0.01,0.49,1.0], [0.03,0.0,0.99], [1.0,0.0,0.96], [1.0,0.0,0.49], [1.0,0.0,0.02]],
        ...
        col => [[0.49,0.0,0.7],[0.45,0.0,0.85],[0.42,0.15,0.89],[0.38,0.29,0.93],[0.27,0.57,0.91], [0.0,0.8,0.77], [0.0,0.97,0.57], [0.0,0.98,0.46],[0.0,1.0,0.35], [0.16, 1.0,0.03], [0.58,1.0,0.0]],
        ...
        jet => [[0.0,0.0,0.51], [0.0,0.24,0.67], [0.01,0.49,0.78], [0.01,0.75,0.89],[0.02,1.0,1.0], [0.51,1.0,0.5], [1.0,1.0,0.0], [0.99,0.67,0.0], [0.99,0.33,0.0], [0.98,0.0,0.0], [0.5,0.0,0.0]],
    };
    colors
}

pub fn color_lerp(colormap_name: &ColorMapSchemes, min: f32, max: f32, mut t: f32) -> [f32; 3] {
    ...
    if index as f32 == 10.0 * tn {
        ...
        let tn_rescaled = (tn - 0.1 * index as f32) * 10.0; // rescale
        let a = colors[index];
        let b = colors[index + 1];
        let color_r = a[0] + (b[0] - a[0]) * tn_rescaled;
        let color_g = a[1] + (b[1] - a[0]) * tn_rescaled;
        let color_b = a[2] + (b[2] - a[2]) * tn_rescaled;
